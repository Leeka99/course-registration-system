...
@Slf4j
class ApplyServiceOptimisticLockTest {
    ...
    @Autowired
    private ApplyServiceOptimisticLock applyServiceOptimisticLock;
    ...
    @DisplayName("낙관적 락을 사용했을 때 데드락이 발생하는지 테스트 - DB 락(낙관적 락)")
    @Test
    void 동시에_신청하는_경우_ReentrantLock_적용() throws InterruptedException {
        ...
        for (int index = 0; index < threadCount; index++) {
            ...
            executorService.submit(() -> {
                try {
                    // 모든 스레드가 여기서 대기
                    startLatch.await();
                    applyServiceOptimisticLock.apply(students.get(idx).getId(),
                        course.getId());
        ...
        Assertions.assertThat(firstGradeNumber).isNotEqualTo(90);
        Assertions.assertThat(thirdGradeNumber).isNotEqualTo(10);

        log.info("firstGradeNumber : {}", firstGradeNumber);
        log.info("thirdGradeNumber : {}", thirdGradeNumber);
    }

    @DisplayName("모든 학년이 신청하는 경우에도 데드락이 발생하는지 테스트한다. - DB 락(낙관적 락)")
    @Test
