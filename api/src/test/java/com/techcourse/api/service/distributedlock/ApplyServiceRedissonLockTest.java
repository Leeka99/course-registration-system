...
@Slf4j
class ApplyServiceRedissonLockTest {
    ...
    @Autowired
    private ApplyServiceRedissonLock applyServiceRedissonLock;
    ...
    @DisplayName("1학년 80명, 3학년 80명이 동시에 신청하는 경우 1학년이 먼저 수강신청 되는지 테스트한다 - RedissonLock")
    @Test
    void 동시에_신청하는_경우_RedissonLock_적용() throws InterruptedException {
        ...
        for (int index = 0; index < threadCount; index++) {
            ...
            executorService.submit(() -> {
                try {
                    ...
                    applyServiceRedissonLock.apply(firstGradeStudents.get(idx).getId(),
                        course.getId());
                    applyServiceRedissonLock.apply(otherGradeStudents.get(idx).getId(),
                        course.getId());
